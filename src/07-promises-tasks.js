...
function chainPromises(array, action) {
  Promise.allSettled = (promises) => Promise.all(promises.map((p) => Promise.resolve(p)
    .then((value) => ({
      state: 'fulfilled',
      value,
    }), (error) => ({
  ...
  return Promise.allSettled(array).then((item) => item.filter((data) => data.status === 'fulfilled')).then((item) => item.map((data) => data.value)).tnen((item) => action(item));
}
